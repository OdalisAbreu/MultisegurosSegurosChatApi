...
if ($_POST) {
	...
	$url = "https://multiseguros.com.do/ws_dev/Seguros/GET_Seguro.php" .
		...
		"&veh_chassis=" . trim(strtoupper($_POST['chassis'])) .
		"&vigencia_poliza=" . trim($_POST['vigencia_poliza']) .
		"&veh_matricula=" . trim(strtoupper($_POST['placa'])) .
	...
	*/
		...
		mysql_query("UPDATE seguro_transacciones SET monto = '" . $monto . "', totalpagar = '" . $monto . "' WHERE id = " . $respuesta[2] . "");
		//	exit('18/1- '. $respuesta[1] .' 2- '. $respuesta[2] .' 3- '. $respuesta[3] .' 4- '. $respuesta[4] .'/00');
