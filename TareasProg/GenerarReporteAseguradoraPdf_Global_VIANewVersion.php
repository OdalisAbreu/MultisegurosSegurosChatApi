...
$directorio = "https://multiseguros.com.do/Seg_V2Dev/images";
...
$quer1 = mysql_query(
	"
	SELECT id FROM personal WHERE id_dist ='" .
		$_GET['user_id'] .
		"'"
);
...
foreach ($array as $key => $val) {
	foreach ($val as $key => $row) {
		...
		$QClient = mysql_query(
			"select * from seguro_clientes WHERE id ='" .
				$row['id_cliente'] .
				"' LIMIT 1"
		);
...
<table width="' .
  ...
  <tr>
    <td  valign="top" align="left"><b>AGENCIA: ' .
			$Agent[0] .
			' - ' .
			strtoupper(sanear_string(utf8_encode($Agencia[0]))) .
			'</b></td>
  ...
  <tr>
    <td  valign="top" align="left" colspan="2"><b>' .
			strtoupper(Remplazar($Agencia[1])) .
			'</b></td>
  </tr>
  <tr>
...
&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<table align="center" cellpadding="2" style="margin-top:24px; margin-left:50px !important;" width="640px">
                
				<tr>
					<td> 
					
					
					<table align="center" cellpadding="3" border="0" style="font-size:25px;">
    ...
    <tr>
        <td align="left" width="90"><b>NO. POLIZA:</b></td>
        <td align="left" width="224"><b>' .
			$poliza .
			'</b></td>
    </tr>
    <tr>
...
if ($html !== '0') {
	...
	$pdf->Output(
		"PDF/CLIENTES/" . $_GET['user_id'] . "/" . $nombreFile . $rutids . ".pdf",
		'F'
	);
	$dir =
		"PDF/CLIENTES/" . $_GET['user_id'] . "/" . $nombreFile . $rutids . ".pdf";
	...
	if (file_exists($dir)) { ?>
		...
		<a href="javascript:void(0)" class="btn btn-success" onclick="location.replace('../ws_dev/TareasProg/Descargar.php?fecha=<?= $nombreFile .
